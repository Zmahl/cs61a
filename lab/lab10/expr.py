...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self_string in env.keys():
            return env[self_string]
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        function = Name.eval(self.operator, env)

        # Convert each operand into it's data type using .eval
        for val in range(0, len(self.operands)):

            if self.operands != Number:
                self.operands[val] = self.operands[val].eval(env)

        return function.apply(self.operands)
