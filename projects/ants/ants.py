...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        #While the current place isn't a beehive, check if there are any bees then go to next place
        while(not curr_place.is_hive):
            # .bees checks a list of bees withing the current place
            if len(curr_place.bees) > 0 and self.min_range <= range <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        #Checks reduce_health on bees, then will implement reduce_health on the FireAnt
        if self.health - amount <= 0:
            for bee in copy_bees:
                # Using reduce health function from Insect superclass, will automatically remove from place, but must iterate over copy
                bee.reduce_health(self.damage + amount)
        
        # Damages every bee in the Place of the FireAnt for the damage the FireAnt took
        else:
            for bee in copy_bees:
                bee.reduce_health(amount)

        # Use the super().reduce_health to call the function's logic from the super class then adjust it here, self parameter not needed
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt blocks bees with large health pool"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt will devour whole bee instantly"""
    ...
    def action(self, gamestate):
        """Eat random bee in current Place."""
